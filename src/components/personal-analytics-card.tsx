...
import {type AnalyticsData, formatDuration} from "@/lib/analytics"
...
export function PersonalAnalyticsCard({volunteerId}: PersonalAnalyticsCardProps) {
    ...
    const fetchAnalytics = async () => {
        try {
            setLoading(true)
            setError(null)
            const response = await fetch(`/api/analytics?userId=${volunteerId}`)
    ...
    const {weeklyStats, monthlyStats, officeBuddies} = analytics
        ...
        <>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="p-4 rounded-lg border bg-card">
                            ...
                            <p className="text-2xl font-bold">
                                {formatDuration(weeklyStats.averageSessionMinutes)}
                            </p>
                        </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Volunteers you&apos;ve spent the most time with
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {officeBuddies.length > 0 ? (
                        <div className="space-y-2">
                            {officeBuddies.map((buddy, idx) => (
                                <div
                                    key={buddy.volunteerId}
